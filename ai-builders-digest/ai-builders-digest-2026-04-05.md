# AI Builders Digest - 2026年04月05日

> Auto-generated by OpenClaw | Data Source: Follow Builders Project

---

## 📊 今日概览 / Today's Overview

- **推文 / Tweets**: 18 条更新
- **播客 / Podcasts**: 0 条更新

---

## 🐦 AI Builders 最新动态 / Latest from AI Builders

### 1. @karpathy

**Andrej Karpathy** · 04-04 23:28

Farzapedia, personal wikipedia of Farza, good example following my Wiki LLM tweet.

Farzapedia，Farza 的个人维基，是我那条"维基 LLM"推文的优秀实践案例。

I really like this approach to personalization in a number of ways, compared to "status quo" of an AI that allegedly gets better the more you use it or something:

相比当下 AI"越用越好"的叙事，我非常喜欢这种个性化方案，原因有以下几点：

1. Explicit. The memory artifact is explicit and navigable (the wiki), you can see exactly what the AI does and does not know and you can inspect and manage this artifact, even if you don't do the direct text writing (the LLM does). The knowledge of you is not implicit and unknown, it's explicit and viewable.
1. **显式化 / Explicit**。记忆制品是显式且可导航的（维基），你能精确看到 AI 知道什么、不知道什么，也可以检查和管理这个制品，哪怕你并不亲自写文本（LLM 代劳）。关于你的知识不是隐含未知的，而是显式可见的。

2. Yours. Your data is yours, on your local computer, it's not in some particular AI provider's system without the ability to extract it. You're in control of your information.
2. **私有化 / Yours**。你的数据是你的，在你自己的电脑上，不被困在某个 AI 提供商系统里无法导出。你对自己的信息拥有完全掌控。

3. File over app. The memory here is a simple collection of files in universal formats (images, markdown). This means the data is interoperable: you can use a very large collection of tools/CLIs or whatever you want over this information because it's just files. The agents can apply the entire Unix toolkit over them. They can natively read and understand them. Any kind of data can be imported into files as input, and any kind of interface can be used to view them as the output. E.g. you can use Obsidian to view them or vibe code something of your own. Search "File over app" for an article on this philosophy.
3. **文件优先于应用 / File over app**。这里的记忆就是通用格式文件（图片、markdown）的简单集合。这意味着数据是互通的：你可以用大量工具/CLI 来处理这些信息，因为它们本质就是文件。Agent 能原生运用整套 Unix 工具链来操作它们，原生读取和理解它们。任何类型的数据都可以导入为文件作为输入，任何类型的界面都可以用来查看它们作为输出。例如你可以用 Obsidian 查看，或者 vibe code 一个自己的界面。搜索"File over app"可以了解更多关于这一理念的文章。

4. BYOAI. You can use whatever AI you want to "plug into" this information - Claude, Codex, OpenCode, whatever. You can even think about taking an open source AI and finetuning it on your wiki - in principle, this AI could "know" you in its weights, not just attend over your data.
4. **自带 AI / BYOAI**。你可以用任何你喜欢的 AI"插入"这些信息——Claude、Codex、OpenCode，随便你。你甚至可以想象用一个开源 AI 在你的维基上微调它——原则上，这个 AI 可以"了解"你，在它的权重里，而不只是对你的数据做注意力运算。

So this approach to personalization puts *you* in full control. The data is yours. In Universal formats. Explicit and inspectable. Use whatever AI you want over it, keep the AI companies on their toes! :)

所以这种个性化方案让你*完全掌控*。数据是你的。用通用格式。显式且可检查。用任何你喜欢的 AI 来处理它，让 AI 公司保持警惕！:)

Certainly this is not the simplest way to get an AI to know you - it does require you to manage file directories and so on, but agents also make it quite simple and they can help you a lot. I imagine a number of products might come out to make this all easier, but imo "agent proficiency" is a CORE SKILL of the 21st century. These are extremely powerful tools - they speak English and they do all the computer stuff for you. Try this opportunity to play with one.

当然，这不是让 AI 了解你最简单的方式——确实需要你管理文件目录等，但 Agent 也让这件事变得相当简单，它们能帮你很多。我预计会有很多产品出现来让这些变得更简便，但在我个人看来，"Agent 能力"是 21 世纪的**核心技能**。这些是非常强大的工具——它们说英语，帮你做所有电脑上的事。找个机会去试试吧。

🔗 [View Original](https://x.com/karpathy/status/2040572272944324650)
❤️ 4186 · 🔁 392

---

### 2. @karpathy

**Andrej Karpathy** · 04-04 21:57

Something I've been thinking about - I am bullish on people (empowered by AI) increasing the visibility, legibility and accountability of their governments.

我一直思考的一件事——我看好人们（在 AI 的赋能下）提高政府的透明度、可理解性和问责度。

Historically, it is the governments that act to make society legible (e.g. "Seeing like a state" is the common reference), but with AI, society can dramatically improve its ability to do this in reverse. Government accountability has not been constrained by access (the various branches of government publish an enormous amount of data), it has been constrained by intelligence - the ability to process a lot of raw data, combine it with domain expertise and derive insights.

从历史上看，是政府在推动让社会变得可理解（比如《Seeing like a state》是经典参考文献），但有了 AI，社会可以大幅提升反过来做这件事的能力。政府问责制过去受制的从来不是获取渠道（政府各分支发布了大量数据），而是智能——即处理大量原始数据、结合领域专业知识并得出洞察的能力。

As an example, the 4000-page omnibus bill is "transparent" in principle and in a legal sense, but certainly not in a practical sense for most people. There's a lot more like it: laws, spending bills, federal budgets, freedom of information act responses, lobbying disclosures... Only a few highly trained professionals (investigative journalists) could historically process this information. This bottleneck might dissolve - not only are the professionals further empowered, but a lot more people can participate.

举个例子，那份 4000 页的综合拨款法案在原则和法律意义上是"透明"的，但对大多数人来说实际上显然并非如此。类似的东西还有很多：法律、支出法案、联邦预算、信息自由法案响应、游说披露……历史上只有少数受过高度训练的专业人士（调查记者）能处理这些信息。这个瓶颈可能会消解——不仅专业人士将被进一步赋能，更多的人也能参与进来。

Some examples to be precise: Detailed accounting of spending and budgets, diff tracking of legislation, individual voting trends w.r.t. stated positions or speeches, lobbying and influence (e.g. graph of lobbyist -> firm -> client -> legislator -> committee -> vote -> regulation), procurement and contracting, regulatory capture warning lights, judicial and legal patterns, campaign finance... Local governments might be even more interesting because the governed population is smaller so there is less national coverage: city council meetings, decisions around zoning, policing, schools, utilities...

具体来说，比如：支出和预算的详细账目追踪、立法的 diff 追踪、议员个人投票趋势（相对于其声明立场或演讲）、游说与影响力图谱（从说客→律所→客户→议员→委员会→投票→监管的完整链路）、采购与合同、监管俘获预警信号、司法与法律模式分析、竞选资金……地方政府可能更有意思，因为治理人口更少，媒体关注度也相对较低：市议会会议、分区规划决策、治安、学校、公用事业等决策……

Certainly, the same tools can easily cut the other way and it's worth being very mindful of that, but I lean optimistic overall that added participation, transparency and accountability will improve democratic, free societies.

当然，同样的工具也很容易朝相反方向发力，这一点值得高度警惕，但总体而言我倾向于乐观——更多参与、更高透明度和问责度将会改善民主自由社会。

(the quoted tweet is half-ish related, but inspired me to post some recent thoughts)

（引用推文只有一半相关，但促使我发布了近期思考）

🔗 [View Original](https://x.com/karpathy/status/2040549459193704852)
❤️ 3029 · 🔁 364

---

### 3. @karpathy

**Andrej Karpathy** · 04-04 16:45

Wow, this tweet went very viral!

哇，这条推文爆了！

I wanted share a possibly slightly improved version of the tweet in an "idea file". The idea of the idea file is that in this era of LLM agents, there is less of a point/need of sharing the specific code/app, you just share the idea, then the other person's agent customizes & builds it for your specific needs.

我想用"想法文件"的形式分享一个可能稍加改进的版本。这个想法文件的意义在于：在 LLM Agent 时代，分享具体代码/应用的意义和需求都降低了——你只需分享想法，然后对方的 Agent 会根据你的具体需求来定制和构建。

So here's the idea in a gist format: https://t.co/NlAfEJjtJV

所以这里用 gist 格式分享这个想法：https://t.co/NlAfEJjtJV

You can give this to your agent and it can build you your own LLM wiki and guide you on how to use it etc. It's intentionally kept a little bit abstract/vague because there are so many directions to take this in. And ofc, people can adjust the idea or contribute their own in the Discussion which is cool.

你可以把这个给你的 Agent，它就能帮你构建自己的 LLM 维基，并引导你怎么使用它等等。它有意保持了一点抽象和模糊，因为这个方向有很多可能性。当然，人们也可以调整这个想法或在 Discussion 中贡献自己的版本，这很酷。

🔗 [View Original](https://x.com/karpathy/status/2040470801506541998)
❤️ 17282 · 🔁 1659

---

### 4. @petergyang

**Peter Yang** · 04-04 18:48

Up at 3 am Shanghai time wondering one thing - should I buy a new MacBook Pro or go for the Mac Studio to vibe code and also run local models?

凌晨 3 点睡不着在想一件事——是买新的 MacBook Pro 还是 Mac Studio，既能 vibe code 又能跑本地模型？

🔗 [View Original](https://x.com/petergyang/status/2040501833899364494)
❤️ 14 · 🔁 1

---

### 5. @petergyang

**Peter Yang** · 04-04 13:11

An incredible deep dive on Disneyland from my friend @TrungTPhan  https://t.co/yHMHnOBbJ6

朋友 @TrungTPhan 写了一篇关于迪士尼乐园的精彩深度解析 https://t.co/yHMHnOBbJ6

🔗 [View Original](https://x.com/petergyang/status/2040416994789613713)
❤️ 18 · 🔁 2

---

### 6. @petergyang

**Peter Yang** · 04-04 12:59

Can someone share a screenshot of what OpenClaw with GPT looks like - how much personality is there?

有人能分享一下 OpenClaw 接 GPT 是什么样子的截图吗——有多强的个性？

🔗 [View Original](https://x.com/petergyang/status/2040414021091037685)
❤️ 63 · 🔁 0

---

### 7. @thenanyu

**Nan Yu** · 04-05 05:15

Design engineers https://t.co/dZeLybBMJq

设计工程师 https://t.co/dZeLybBMJq

🔗 [View Original](https://x.com/thenanyu/status/2040659521337504215)
❤️ 3 · 🔁 0

---

### 8. @thenanyu

**Nan Yu** · 04-05 01:27

https://t.co/zzfwvC3401

🔗 [View Original](https://x.com/thenanyu/status/2040602311148404890)
❤️ 0 · 🔁 0

---

### 9. @thenanyu

**Nan Yu** · 04-05 01:10

https://t.co/kv205MYqCC https://t.co/fvee5JZe9i

🔗 [View Original](https://x.com/thenanyu/status/2040597900502135059)
❤️ 9 · 🔁 0

---

### 10. @trq212

**Thariq** · 04-04 22:30

POV: you're cooking https://t.co/Aw546zNBq7

视角：你在做饭 https://t.co/Aw546zNBq7

🔗 [View Original](https://x.com/trq212/status/2040557561464885382)
❤️ 722 · 🔁 10

---

### 11. @levie

**Aaron Levie** · 04-05 04:40

One of the core things we're going to have to contend with in AI is that even the most advanced models in the word can’t have all the relevant knowledge needed to be useful, because everyone has different use-cases and ways they've designed their workflows.

AI 领域我们不得不面对的一个核心问题是：哪怕世界上最先进的模型也无法拥有所有相关知识来做到真正有用，因为每个人的用例和工作流设计都各不相同。

Perhaps most importantly, as you get into the enterprise, everyone has entirely different access levels to corporate knowledge and information. Continual learning at the model layer, even at a single enterprise level, is near impossible because every user knows and has access to something different than another user.

更重要的是，进入企业领域后，每个人对企业知识和信息的访问权限完全不同。在模型层面进行持续学习几乎是不可能的——哪怕在单一企业层面——因为每个用户知道和访问的东西都与其他用户不同。

This isn't like coding where by and large most developers can access all the relevant stuff to their job. On a single banking team, bankers have entirely different sets of documents they're ever allowed to see. Sanitizing this is hard and having the model keep secrets is impossible.

这和编程不一样——大体上大多数开发者能访问所有与工作相关的东西。在一个银行团队里，银行家们能看的文件集合完全不同。清洗这些数据很难，而让模型保密则根本不可能。

This is why the context layer is going to always be the core part of the AI stack for applied use cases to turn general models turn into useful agents. Can't fight the physics on this one.

这就是为什么 Context 层将始终是 AI 堆栈中将通用模型转化为有用 Agent 的核心部分。这一点上物理规律无法对抗。

🔗 [View Original](https://x.com/levie/status/2040650799550722243)
❤️ 99 · 🔁 12

---

### 12. @levie

**Aaron Levie** · 04-04 20:35

As AI models get better at handling tools, and as context windows get bigger without as much rot, you can start to design agents more similar to how people work instead of having to mitigate the model limitations with weird hacks.

随着 AI 模型在工具调用上越来越强，上下文窗口越来越大且衰减不那么严重，你现在可以开始设计更像人一样工作的 Agent 了，而不必用奇怪的 hack 来弥补模型的局限性。

For instance, even a year ago, if you were to build an agent to process large amounts of documents, the state of the art was to do embeddings on the data, then do a similarity search and pull out the chunks of content that matched (as well as surrounding chunks). This was necessary because context windows could only accurately handle a small amount of information at a time.

举个例子，哪怕在一年前，如果你要构建一个处理大量文档的 Agent，最先进的方法是对数据做 Embedding，然后做相似性搜索并提取匹配的内容块（以及周围的块）。这是必要的，因为上下文窗口一次只能准确处理少量信息。

This worked surprisingly well given the constraints (at least assuming you were working with权威数据 only), but had a lot of tricky limitations because it's not how humans work.

考虑到这些限制，这个方法效果出奇地好（至少在只处理权威数据的情况下），但也有很多棘手的局限性，因为它并不符合人类的工作方式。

For instance, what do you do if the chunks you sent to the model were the most relevant semantically, but actually rendered irrelevant by some other part of the document. For instance, if at the top of the document it says "do not use this" but on page 3 there is information that's relevant, that data will be sent to the model as the top hit. Similarly, chunked data is difficult when you need various parts of a document or many documents to be understood for answering a problem.

举个例子，如果你发给模型的内容块在语义上最相关，但被文档另一部分的信息推翻了呢？比如文档开头说"不要使用这个"，但第 3 页有一条相关信息，那条数据仍会被作为最高匹配发送给模型。同样，当需要理解文档的多个部分或许多文档才能回答问题时，分块数据就很难处理。

Today, increasingly, you can begin to have agents effectively use tools and work with information far more similar to how people work. This unlocks a qualitatively different set of use-cases and capability level that agents can now handle.

如今，越来越多的场景中，你已经可以让 Agent 有效地使用工具和处理信息，工作方式越来越接近人类。这解锁了一套全新的、质的不同的用例和能力级别。

As we were designing the Box Agent, these improvements allowed us to rethink our entire architecture for AI. The agent can now search data similar to how a user searches, but with the benefit of being able to expand their queries, do semantic search, and process results nearly instantly. Then the agent can either read many documents at a time or at least much larger amounts of context. Again, much more similar to people, but now at hyperspeed.

在我们设计 Box Agent 时，这些改进让我们得以重新思考整个 AI 架构。Agent 现在可以像用户一样搜索数据，但同时能够扩展查询词、做语义搜索、近乎即时地处理结果。然后 Agent 可以一次读取许多文档，或者至少大得多的上下文量。同样更接近人类了——但现在是以超速度。

Importantly, beyond tool calling and context windows, the reasoning of models has also gone up enormously. This means the agent can also know when it needs to search for information again when it didn't find something it was looking for or if something feels off.

重要的是，除了工具调用和上下文窗口，模型的推理能力也大幅提升了。这意味着 Agent 现在也能知道什么时候需要重新搜索信息——当它没找到要找的东西或感觉哪里不对的时候。

As model progress continues on the dimensions of context accuracy, tool calling, advanced reasoning, and coding, agents are going to become insanely powerful.

随着模型在上下文准确性、工具调用、高级推理和编程这些维度上持续进步，Agent 将变得极其强大。

🔗 [View Original](https://x.com/levie/status/2040528801386361128)
❤️ 98 · 🔁 12

---

### 13. @garrytan

**Garry Tan** · 04-05 05:47

Also trying to make reviews adaptive and smarter. More work towards L8 software factory. Slow and steady. https://t.co/k8VR3v497U

同时也在努力让 Code Review 变得更自适应和智能。L8 软件工厂的更多工作在推进。慢而稳。 https://t.co/k8VR3v497U

🔗 [View Original](https://x.com/garrytan/status/2040667535490863254)
❤️ 5 · 🔁 2

---

### 14. @garrytan

**Garry Tan** · 04-05 05:12

14 security bug fixes just landed for GStack, half of which were community PR's. https://t.co/98jmCzQ38i

GStack 一口气修复了 14 个安全 bug，其中一半是社区的 PR。 https://t.co/98jmCzQ38i

🔗 [View Original](https://x.com/garrytan/status/2040581964156342768)
❤️ 61 · 🔁 1

---

### 15. @garrytan

**Garry Tan** · 04-05 00:07

We are fighting to stop this

Join @garryslist if you want to help us https://t.co/2iKxFtZbtE

我们正在努力阻止这件事

如果你想帮忙，加入 @garryslist https://t.co/2iKxFtZbtE

🔗 [View Original](https://x.com/garrytan/status/2040581964156342768)
❤️ 61 · 🔁 5

---

### 16. @zarazhangrui

**Zara Zhang** · 04-04 23:03

Prompting tip from @steipete:

I always ask the model, "Do you have any questions?"

来自 @steipete 的 Prompt 技巧：

我总是问模型："你有什么问题吗？"

🔗 [View Original](https://x.com/zarazhangrui/status/2040565939100676382)
❤️ 71 · 🔁 2

---

### 17. @nikunj

**Nikunj Kothari** · 04-04 19:33

Don't forget to smell the flowers 💐

别忘了闻闻花香 💐

🔗 [View Original](https://x.com/nikunj/status/2040513012440895664)
❤️ 30 · 🔁 0

---

### 18. @steipete

**Peter Steinberger** · 04-04 18:11

"There's a big wave coming" https://t.co/sbGnugb3Hz

"有一波大浪即将到来" https://t.co/sbGnugb3Hz

🔗 [View Original](https://x.com/steipete/status/2040492573819904026)
❤️ 1043 · 🔁 83

---

## 🎙️ 播客更新 / Podcasts Updates

### 1. AI for Atoms: How Periodic Labs is Revolutionizing Materials Engineering with Co-Founder Liam Fedus

**Channel**: Unknown
**Link**: https://www.youtube.com/@NoPriorsPodcast

Speaker 1 | 00:05 - 00:32
Today, know Priors were talking with Liam Fetis. Liam is one of the co creators of ChatGPT, which I think almost everybody uses at this point. He was the VP of post training at OpenAI, and before that was at Google Brain, where he worked on a variety of really early AI inno...

今天，know Priors 和 Liam Fetis 聊了聊。Liam 是 ChatGPT 的共同创始人之一，我想现在几乎所有人都在用 ChatGPT。他是 OpenAI 的后训练副总裁，在此之前在 Google Brain 工作，从事各种非常早期的 AI 创新……

---

## 📚 资源 / Resources

- **Follow Builders Project**: https://github.com/zarazhangrui/follow-builders
- **更新频率 / Update Frequency**: 推文每 6 小时一次，播客每 24 小时一次

---

*Generated: 2026-04-05 23:08:05*
*By OpenClaw AI Builders Digest*
